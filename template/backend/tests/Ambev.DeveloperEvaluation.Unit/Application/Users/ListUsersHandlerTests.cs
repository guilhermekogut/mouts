...
public class ListUsersHandlerTests
{
    ...
    public async Task Handle_ValidRequest_ReturnsMappedResult()
    {
        ...
        var expectedItems = new List<ListUserItemResult>
        {
            new ListUserItemResult
            {
                ...
                Id = users[0].Id,
                Name = new DeveloperEvaluation.Application.Users.Common.NameResult
                {
                    Firstname = "John",
                    Lastname = "Doe"
                },
                ...
                Role = users[0].Role,
                Address = new DeveloperEvaluation.Application.Users.Common.AddressResult
                {
                    City = "City",
                    Street = "Street",
                    Number = 123,
    ...
    public async Task Handle_AddressNumberRangeFilter_ReturnsUsersInRange()
    {
        ...
        {
            var filtered = callInfo.ArgAt<IEnumerable<User>>(0).Where(u => u.Address.Number >= 2 && u.Address.Number <= 3);
    ...
    public async Task Handle_OrderByEmailDesc_ReturnsUsersInOrder()
    {
        ...
        _mapper.Map<IEnumerable<ListUserItemResult>>(Arg.Any<IEnumerable<User>>()).Returns(callInfo =>
        {
            var ordered = callInfo.ArgAt<IEnumerable<User>>(0).OrderByDescending(u => u.Email);
    ...
    public async Task Handle_Pagination_ReturnsCorrectPage()
    {
        ...
        _mapper.Map<IEnumerable<ListUserItemResult>>(Arg.Any<IEnumerable<User>>()).Returns(callInfo =>
        {
            var input = callInfo.ArgAt<IEnumerable<User>>(0);
